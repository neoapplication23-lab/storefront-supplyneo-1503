...
export default function Hero({ data, departureTime }) {
      ...
      <section style={{
        ...
        <div style={{ position: 'relative', zIndex: 2, maxWidth: 1100, margin: '0 auto', width: '100%', padding: '16px clamp(16px,4vw,36px) 18px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
            ...
            <span style={{ fontSize: 9.5, fontWeight: 600, letterSpacing: '.14em', textTransform: 'uppercase', color: 'rgba(255,255,255,.6)' }}>{eyebrowText}</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              {hasLogo && <img src={logo} alt={businessName} onError={() => setLogoErr(true)} style={{ height: 32, objectFit: 'contain', filter: 'brightness(0) invert(1)' }} />}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
              {chips.map((c, i) => (
                <span key={i} style={{
                  ...
                  borderRadius: 'var(--r-pill)', background: 'rgba(255,255,255,.12)',
  ...
  // Without image: original compact inline style
  return (
    <section style={{ borderBottom: '1px solid rgba(0,0,0,.07)', background: '#fff', padding: '18px clamp(16px,4vw,36px) 16px' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          {hasLogo && <img src={logo} alt={businessName} onError={() => setLogoErr(true)} style={{ height: 40, objectFit: 'contain' }} />}
          <div>
            {businessName && (
              <p style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-muted)', letterSpacing: '.08em', textTransform: 'uppercase', marginBottom: 2 }}>{businessName}</p>
            )}
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
              <div style={{ width: 12, height: 1.5, background: pc }} />
              <span style={{ fontSize: 9.5, fontWeight: 600, letterSpacing: '.12em', textTransform: 'uppercase', color: pc }}>{eyebrowText}</span>
            </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          {chips.map((c, i) => (
            <span key={i} style={{
              ...
              borderRadius: 'var(--r-pill)', background: `${pc}0d`, border: `1px solid ${pc}22`,
